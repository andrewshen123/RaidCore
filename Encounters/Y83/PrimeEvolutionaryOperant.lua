...
mod:RegisterDefaultSetting("DangerZone", false)
...
function mod:OnBossEnable()
	...
	if mod:GetSetting("DangerZone") then
		local Vector = INCUBATION_REGROUP_ZONE[INCUBATION_ZONE_NORTH]
		core:AddPolygon("DZ" .. INCUBATION_ZONE_NORTH, Vector, 11, 0, 3, "xkcdBrightRed", 16)
	end
end
...
function mod:OnDebuffAdd(nId, nSpellId, nStack, fTimeRemaining)
    ...
    if DEBUFF_STRAIN_INCUBATION == nSpellId then
		...
		if not first_inc then
			first_inc = true
			--first incubation timer add
			mod:AddTimerBar("SI_CD", "Incubation Time Remaining", 40, mod:GetSetting("SoundNextIrradiateCountDown"))
		end
    ...
    if BUFF_COMPROMISED_CIRCUITRY == nSpellId then
        ...
        if not bIsPhaseUnder20Poucent then
            ...
            if mod:GetSetting("DangerZone") then
                ...
                if nIndex then
                    local Vector = INCUBATION_REGROUP_ZONE[nIndex]
					core:AddPolygon("DZ" .. nIndex, Vector, 11, 0, 3, "xkcdBrightRed", 16)
                end
            end
        end
    end
