...
local RAIDCORE_CURRENT_VERSION = "5.23"
-- Should be deleted.
...
local function OnEncounterHookGenericUnsafe(sMethod, ...)
    local fEncounter = _tCurrentEncounter[sMethod]
    if fEncounter then
        fEncounter(_tCurrentEncounter, ...)
	else
...
local function ProcessDelayedUnit()
    ...
    if not pcall(OnEncounterHookGenericUnsafe, "PersistDelayedUnits") then
		_tDelayedUnits = {}
	end
end
