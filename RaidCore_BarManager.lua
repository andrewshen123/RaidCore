...
local function UpdateCustomBar(tUnitManager, tBar)
    local fCallback = tBar.fCallback
    if fCallback then
        local MaxValue = tBar.nMax
        local Value = fCallback()
        local sName = tBar.sKey
        if Value and MaxValue then
            local nPourcent = 100.0 * Value / MaxValue
            if tBar.wndUnitHPProgressBar then
                -- Update progress bar.
                tBar.wndUnitHPProgressBar:SetMax(MaxValue)
                tBar.wndUnitHPProgressBar:SetProgress(Value)
				if tBar.sColor then
					tBar.wndUnitHPProgressBar:SetBarColor(tBar.sColor)
				else
					tBar.wndUnitHPProgressBar:SetBarColor("FFFF9933")
				end
                tBar.wndUnit:SetBGColor("A0131313")
                -- Update the percent text.
                local sPourcentFormat = "%.1f%%"
                ...
                tBar.wndUnitHPPercent:SetText(sPourcentFormat:format(nPourcent))
                -- Update the short health text.
                tBar.wndUnitHPValue:SetText(Number2ShortString(Value))
            end
        elseif tBar.wndUnitHPPercent then
            tBar.wndUnitHPPercent:SetText("")
            tBar.wndUnitHPValue:SetText("")
...
-- CustomManager Class.
----------------------------------------------------------------------------------------------------
local CustomManager = NewManager("Custom")

function CustomManager:AddBar(sKey, nMin, nMax, fCallback, sColor)
    if not self.tBars[sKey] then
        local sMark = nil
        if self.tSettings.bEnabled then
            local wndMain = Apollo.LoadForm(RaidCore.xmlDoc, "BarUnitTemplate", self.wndParent, self)
            local wndBody = wndMain:FindChild("Body")
            ...
            wndMain:SetData(GetGameTime())
            wndMain:SetAnchorOffsets(0, 0, 0, self.tSettings.nBarHeight)
            UpdateCustomBar(self, self.tBars[sKey])
            ArrangeBar(self)
        else
            self.tBars[sKey] = {
                sKey = sKey,
...
function CustomManager:OnTimerUpdate()
    for _, Bar in next, self.tBars do
        UpdateCustomBar(self, Bar)
    end
end
...
function RaidCore:AddCustom(sKey, nMin, nMax, fCallback, sColor)
	CustomManager:AddBar(sKey, nMin, nMax, fCallback, sColor)
end

function RaidCore:RemoveCustom(nId)
    CustomManager:RemoveBar(nId)
end
