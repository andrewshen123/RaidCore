    ...
    if tDraw.nOriginId then
        ...
        if tOriginUnit and tOriginUnit:IsValid() then
			...
			if tDraw.vOffsets ~= nil then
				...
				tVectorFrom.x = tVectorFrom.x + tDraw.vOffsets.x * math.cos(nTheta) - tDraw.vOffsets.z * math.sin(nTheta)
				tVectorFrom.y = tVectorFrom.y + tDraw.vOffsets.y
				tVectorFrom.z = tVectorFrom.z + tDraw.vOffsets.z * math.cos(nTheta) + tDraw.vOffsets.x * math.sin(nTheta)
			end
        end
    else
...
function SimpleLine:AddOffsetDraw(Key, Origin, vOffsets, nLength, nRotation, nWidth, sColor, nNumberOfDot)
    local OriginType = type(Origin)
    if OriginType ~= "number" then
		error("Use AddOffsetLine only with a unit id as the origin")
	end
    if self.tDraws[Key] then
        -- To complex to manage new definition with nNumberOfDot which change,
        -- simplest to remove previous.
...
function RaidCore:AddOffsetLine(...)
    return SimpleLine:AddOffsetDraw(...)
end
